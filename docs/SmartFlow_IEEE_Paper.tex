...
The \textbf{Redis Cache} is utilized to store ephemeral agent states and recently computed forecasts, significantly reducing database load during high-concurrency "Health Sweeps". This cache invalidation strategy follows a time-to-live (TTL) policy synchronized with the ledger update frequency (typically daily).
